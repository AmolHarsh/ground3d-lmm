...
"""Build the Reason3D ``infos`` pkls (train / val).

Reads the pre-processed ScanNet data (``scannet_instance_data/``), the
per-scene Reason3D reasoning questions and the per-scene ``<scene>_reason.pth``
...
def reason3d_data_prep(root_path, qa_data_dir, reason_pth_dir, info_prefix,
                       out_dir, workers):
    """Generate Reason3D info files for the train and val splits.

    Args:
        root_path (str): Root of the pre-processed ScanNet data.
        qa_data_dir (str): Directory holding the Reason3D by-scene JSONs.
    ...
    from part3d_glamm_data_utils import Reason3dData

    save_path = out_dir

    for split in ('train', 'val'):
        filename = osp.join(save_path, f'{info_prefix}_infos_{split}.pkl')
        dataset = Reason3dData(
            root_path=root_path,
...
def parse_args():
    parser = argparse.ArgumentParser(description='Reason3D data converter')
    parser.add_argument(
        '--root-path',
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help='directory holding reason3d_scannet_{split}_by_scene.json')
    parser.add_argument(
        '--reason-pth-dir',
        type=str,
        required=True,
