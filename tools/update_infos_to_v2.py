...
def update_part3d_glamm_infos(pkl_path, out_dir):
    print(f'{pkl_path} will be modified.')
    if out_dir in pkl_path:
        print(f'Warning, you may overwriting '
              f'the original data {pkl_path}.')
    ...
    for ori_info_dict in mmengine.track_iter_progress(data_list):
        ...
        if 'pts_part_mask_path' in ori_info_dict:
            temp_data_info['pts_part_mask_path'] = Path(
                ori_info_dict['pts_part_mask_path']).name
        if 'pts_object_mask_path' in ori_info_dict:
            temp_data_info['pts_object_mask_path'] = Path(
                ori_info_dict['pts_object_mask_path']).name
        if 'super_pts_path' in ori_info_dict:
            temp_data_info['super_pts_path'] = Path(
                ori_info_dict['super_pts_path']).name
        if 'qa_data' in ori_info_dict:
            temp_data_info['qa_data'] = ori_info_dict['qa_data']
...
def update_reason3d_infos(pkl_path, out_dir):
    print(f'{pkl_path} will be modified.')
    if out_dir in pkl_path:
        print(f'Warning, you may overwriting '
              f'the original data {pkl_path}.')
...
def update_pkl_infos(dataset, out_dir, pkl_path):
    ...
    elif dataset.lower() == 'part3d_glamm' or dataset.lower() == 'part3d_glamm_scannetpp':
        update_part3d_glamm_infos(pkl_path=pkl_path, out_dir=out_dir)
    elif dataset.lower() == 'reason3d':
        update_reason3d_infos(pkl_path=pkl_path, out_dir=out_dir)
    elif dataset.lower() == 'instruct3d':
        update_reason3d_infos(pkl_path=pkl_path, out_dir=out_dir)
    else:
