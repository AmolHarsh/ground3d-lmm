...
class Det3DDataPreprocessor_(Det3DDataPreprocessor):
    def simple_process(self, data, training=False):
        ...
        if 'scene_name' in inputs:
            batch_inputs['scene_name'] = inputs['scene_name']

        # new fields added by Part3D Glamm
        if 'selected_qa_data' in inputs:
            batch_inputs['selected_qa_data'] = inputs['selected_qa_data']
        
        if 'sp_gt_seg' in inputs:
            batch_inputs['sp_gt_seg'] = inputs['sp_gt_seg']
