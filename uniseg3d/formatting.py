...
class Pack3DDetInputs_(Pack3DDetInputs):
    ...
    INPUTS_KEYS = ['points', 'img', 'elastic_coords',
                   ...
                   'emb_text', 'label_text', 'gt_text_prompt','text_token','text_object_id',
    ...
    def pack_single_results(self, results: dict) -> dict:
        ...
        for key in [
                ...
                'point_prompts', 'point_prompt_instance_ids', 'point_prompt_sp_ids',
                'emb_text', 'label_text', 'gt_text_prompt', 'is_novel', 'text_token','text_object_id',
                'pts_instance_objextId_shuffle', 'point_prompt_distance_norms','scene_name'
        ]:
...
@TRANSFORMS.register_module()
class Pack3DDetInputs_Part_Glamm(Pack3DDetInputs):
    """Just add elastic_coords, sp_pts_mask, and gt_sp_masks.
    """
    INPUTS_KEYS = ['points', 'img', 'elastic_coords',
                   'point_prompts', 'point_prompt_instance_ids', 'point_prompt_sp_ids',
    ...
    SEG_KEYS = [
        ...
        'pts_object_mask',
        'sp_gt_seg'
        # 'sp_gt_seg'  # moved to INPUTS_KEYS, no longer used as SEG_KEY
    ]
    INSTANCEDATA_3D_KEYS = [
        'gt_bboxes_3d', 'gt_labels_3d', 'attr_labels', 'depths', 'centers_2d',
        'gt_sp_masks','is_novel'
    ...
    def pack_single_results(self, results: dict) -> dict:
        ...
        for key in [
                ...
                'scene_name','selected_qa_data', 'pts_part_mask', 'pts_object_mask', 'sp_gt_seg'
        ]:
            if key not in results:
                continue
            if isinstance(results[key], list):
                # check whether list elements can be converted to tensor
                if len(results[key]) > 0 and isinstance(results[key][0], (np.ndarray, torch.Tensor)):
                    results[key] = [to_tensor(res) for res in results[key]]
                else:
                    results[key] = results[key]
                # otherwise keep the original format (e.g. list of dicts)
            elif isinstance(results[key], str):
                results[key] = results[key]
            elif isinstance(results[key], dict):
                # dict type stays as-is
                results[key] = results[key]
            elif isinstance(results[key], (np.ndarray, torch.Tensor)):
                # only convert numpy arrays or tensors
                results[key] = to_tensor(results[key])
            else:
            ...
            if key in results:
                if key in self.INPUTS_KEYS:
                    ...
                    if results[key] is None:
                        # print(f"Warning: {key} is None, skipping assignment")
                        # skip None values, PointData does not accept None
                        continue
                    gt_pts_seg[self._remove_prefix(key)] = results[key]
                else:
                    raise NotImplementedError(f'Please modified '
...
@TRANSFORMS.register_module()
class Pack3DDetInputs_Reason3D(Pack3DDetInputs):
    """Just add elastic_coords, sp_pts_mask, and gt_sp_masks.
    """
    INPUTS_KEYS = ['points', 'img', 'elastic_coords',
                   'point_prompts', 'point_prompt_instance_ids', 'point_prompt_sp_ids',
    ...
    def pack_single_results(self, results: dict) -> dict:
        ...
        for key in [
                ...
                'emb_text', 'label_text', 'gt_text_prompt', 'is_novel', 'text_token','text_object_id',
                'pts_instance_objextId_shuffle', 'point_prompt_distance_norms',
                'scene_name','selected_qa_data',  'sp_gt_seg'
        ]:
            if key not in results:
                continue
            if isinstance(results[key], list):
