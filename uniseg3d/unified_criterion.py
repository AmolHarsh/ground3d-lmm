...
class ScanNetUnifiedCriterion(nn.Module):
    def __init__(self, num_semantic_classes, sem_criterion, inst_criterion, 
                 point_prompt_inst_criterion, text_prompt_inst_criterion, contrastive_criterion, cot_criterion=None):
        ...
        if cot_criterion is not None:
            self.cot_criterion = MODELS.build(cot_criterion)
        else:
            self.cot_criterion = None
    
    def __call__(self, pred, insts,
                 point_prompt_pred=None, point_prompt_insts=None,
                 text_prompt_pred=None, text_prompt_insts=None, language_loss=None):
        sem_gts = []
        inst_gts = []
        point_prompt_gts = []
        text_prompt_gts = []
        ...
        if self.cot_criterion is not None and language_loss is not None:
            loss.update(self.cot_criterion(language_loss))
...
@MODELS.register_module()
class ScanNetUnifiedCriterion_OnlyLLM(nn.Module):
    def __init__(self, cot_criterion=None):
        super(ScanNetUnifiedCriterion_OnlyLLM, self).__init__()
        if cot_criterion is not None:
            self.cot_criterion = MODELS.build(cot_criterion)
    
    def __call__(self, pred=None, insts=None,
                 point_prompt_pred=None, point_prompt_insts=None,
                 text_prompt_pred=None, text_prompt_insts=None, language_loss=None):
                
        loss = self.cot_criterion(language_loss)
        return loss
...
@MODELS.register_module()
class Part3DGlammCriterion(nn.Module):
    def __init__(self, lang_seg_criterion, lang_criterion):
        super(Part3DGlammCriterion, self).__init__()
        self.lang_seg_criterion = MODELS.build(lang_seg_criterion)
        self.lang_criterion = MODELS.build(lang_criterion)

    
    def __call__(self, pred, insts, language_loss=None):
        
        loss = self.lang_seg_criterion(pred, insts)
        if self.lang_criterion is not None and language_loss is not None:
            loss.update(self.lang_criterion(language_loss))
...
@MODELS.register_module()
class Part3DGlammCriterion_v2(nn.Module):
    def __init__(self, lang_seg_criterion, lang_criterion):
        super(Part3DGlammCriterion_v2, self).__init__()
        self.lang_seg_criterion = MODELS.build(lang_seg_criterion)
        self.lang_criterion = MODELS.build(lang_criterion)

    
    def __call__(self, pred, insts, language_loss=None, original_queries=None):
        
        loss = self.lang_seg_criterion(pred, insts, original_queries)
        if self.lang_criterion is not None and language_loss is not None:
            loss.update(self.lang_criterion(language_loss))
