...
def get_iou(gt_masks, pred_masks):
    ...
    for gt, pred in zip(gt_masks, pred_masks):
        ...
        # Handle multi-dimensional masks: compute IoU for each instance
        if gt.ndim > 1:
            # Convert to boolean if needed
            valid_gt_index = gt.sum(axis=-1) > 0
            
            gt = gt.astype(bool) if gt.dtype != bool else gt
            pred = pred.astype(bool) if pred.dtype != bool else pred
            
            # Compute IoU for each instance
            intersection = (gt & pred).sum(axis=-1)  # Sum over last dimension for each instance
            union = (gt | pred).sum(axis=-1)  # Sum over last dimension for each instance
            instance_ious = intersection / (union + 1.e-8)
            
            # Filter valid GTs for mean calculation
            if valid_gt_index.sum() > 0:
                iou = np.mean(instance_ious[valid_gt_index])
            else:
                continue
        else:
            # 1D mask: compute IoU directly
            if gt.sum() > 0:
                gt = gt.astype(bool) if gt.dtype != bool else gt
                pred = pred.astype(bool) if pred.dtype != bool else pred
                intersection = (gt & pred).sum()
                union = (gt | pred).sum()
                iou = intersection / (union + 1.e-8)
            else:
                continue
        ious.append(iou)
    ...
    # # Convert to boolean if needed
    # gt_masks = gt_masks.astype(bool) if gt_masks.dtype != bool else gt_masks
    # pred_masks = pred_masks.astype(bool) if pred_masks.dtype != bool else pred_masks
    
    # # Handle multi-dimensional masks: compute IoU for each instance
    # if gt_masks.ndim > 1:
    #     intersection = (gt_masks & pred_masks).sum(axis=-1)  # Sum over last dimension for each instance
    #     union = (gt_masks | pred_masks).sum(axis=-1)  # Sum over last dimension for each instance
    #     ious = intersection / (union + 1.e-8)
    # else:
    #     # 1D mask: compute IoU directly
    #     intersection = (gt_masks & pred_masks).sum()
    #     union = (gt_masks | pred_masks).sum()
    #     ious = intersection / (union + 1.e-8)
    # return ious
...
@METRICS.register_module()
class Part3DGlamm_SegMetric(SegMetric):
    """Metric for instance, semantic, and panoptic evaluation.
    The order of classes must be [stuff classes, thing classes, unlabeled].
    ...
    def process(self, data_batch: dict, data_samples: Sequence[dict]) -> None:
        ...
        for data_sample in data_samples:
            ...
            qa_gt_masks = pred_3d['qa_gt_masks']
            qa_pred_masks = pred_3d['qa_pred_masks']

            # Move to CPU/Numpy if needed to avoid GPU memory issues and for get_iou compatibility
            if isinstance(qa_gt_masks, torch.Tensor):
                qa_gt_masks = qa_gt_masks.cpu().numpy()
            elif isinstance(qa_gt_masks, list) and len(qa_gt_masks) > 0 and isinstance(qa_gt_masks[0], torch.Tensor):
                qa_gt_masks = [m.cpu().numpy() for m in qa_gt_masks]
            
            if isinstance(qa_pred_masks, torch.Tensor):
                qa_pred_masks = qa_pred_masks.cpu().numpy()
            elif isinstance(qa_pred_masks, list) and len(qa_pred_masks) > 0 and isinstance(qa_pred_masks[0], torch.Tensor):
                qa_pred_masks = [m.cpu().numpy() for m in qa_pred_masks]

            if len(qa_gt_masks) == 0:
                cur_sample_prompt_IoUs_qa = np.array([])
            else:
                cur_sample_prompt_IoUs_qa = get_iou(qa_gt_masks, qa_pred_masks)
            
            # Append only the IoU results, not the heavy masks
            self.results.append(cur_sample_prompt_IoUs_qa)
    ...
    def compute_metrics(self, results):
        ...
        # results contains list of IoU arrays from all ranks
        val_set_prompt_IoUs_qa_list = results

        if len(val_set_prompt_IoUs_qa_list) > 0:
            val_set_prompt_IoUs_qa = np.concatenate(val_set_prompt_IoUs_qa_list)
        else:
            val_set_prompt_IoUs_qa = np.array([])

        metrics = dict()

        if len(val_set_prompt_IoUs_qa) > 0:
            pre_half = (val_set_prompt_IoUs_qa > 0.5).sum() / len(val_set_prompt_IoUs_qa)
            pre_quarter = (val_set_prompt_IoUs_qa > 0.25).sum() / len(val_set_prompt_IoUs_qa)
            metrics.update({'QA Seg mIoU':np.mean(val_set_prompt_IoUs_qa),
                            'QA Seg Precision_half': pre_half,
                            'QA Seg Precision_quarter': pre_quarter})
        return metrics
