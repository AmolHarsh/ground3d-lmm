...
class UniSeg3D(UniSeg3DMixin, Base3DDetector):
    ...
    def __init__(self,
        ...
        # Keep original model behavior; do not apply special freezing changes
    ...
    def loss(self, batch_inputs_dict, batch_data_samples, **kwargs):
        ...
        # Compute superpoint centroids (positions) per sample for position-aware aggregation
        raw_positions = torch.vstack([p[:, :3] for p in batch_inputs_dict['points']]).to(sp_pts_masks.device)
        sp_centroids = scatter_mean(raw_positions, sp_pts_masks, dim=0)
        point_positions = [sp_centroids[batch_offsets[i]: batch_offsets[i + 1]] for i in range(len(batch_offsets) - 1)]

        x = self.decoder(x, queries, interaction_masks=interaction_masks, point_positions=point_positions)
...
@MODELS.register_module()
class Grounded_LMM_main(UniSeg3DMixin, Base3DDetector):
    r"""UniSeg3D for ScanNet dataset.
    ...
    def loss(self, batch_inputs_dict, batch_data_samples, **kwargs):
        ...
        for i in range(len(batch_data_samples)):
            ...
            batch_data_samples[i].gt_pts_seg
            ...
            sp_gt_masks.append(torch.cat(batch_data_samples[i].gt_pts_seg.sp_gt_seg, dim=0))
        ...
        scene_names  = batch_inputs_dict['scene_name']
        selected_qa_data = batch_inputs_dict['selected_qa_data']

        x = self.decoder(x, scene_names=scene_names, qa_data=selected_qa_data, point_positions=point_positions)
        language_losses = x['language_losses']
        ...
        loss = self.criterion(x, sp_gt_masks, language_losses, original_queries=x['original_queries'])
    ...
    def qa_predict_by_feat(self, out, superpoints, gt_masks):

        pred_masks = []

        for i_batch in range(len(out['masks'])):
            pred_mask_= []
            for i_qa_pred in range(len(out['masks'][i_batch])):

                mask_pred = out['masks'][i_batch][i_qa_pred]
                mask_pred_sigmoid = mask_pred.sigmoid()
                # mask_pred_sigmoid = mask_pred_sigmoid[:, superpoints]
                mask_pred = mask_pred_sigmoid > self.test_cfg.sp_score_thr
                mask_pred = mask_pred[:, superpoints]
                # GT masks can be fewer than predicted masks: multi-turn produces one prediction per
                # turn, but GT exists only for the grounded turns. Align to GT when it is present;
                # otherwise keep the raw prediction so it is still available for visualization.
                if i_qa_pred < len(gt_masks[i_batch]):
                    gt_masks[i_batch][i_qa_pred]=gt_masks[i_batch][i_qa_pred][:, superpoints.cpu()]

                    if gt_masks[i_batch][i_qa_pred].shape[0]>mask_pred.shape[0]:
                        mask_pred=torch.cat([mask_pred, torch.zeros((gt_masks[i_batch][i_qa_pred].shape[0]-mask_pred.shape[0], mask_pred.shape[1])).to(mask_pred.device)], dim=0)

                    elif gt_masks[i_batch][i_qa_pred].shape[0]<mask_pred.shape[0]:
                        mask_pred=mask_pred[:gt_masks[i_batch][i_qa_pred].shape[0]]

                pred_mask_.append(mask_pred.cpu().numpy())
        ...
        for i in range(len(pred_masks)):
            results.append(PointData(
                qa_pred_masks=pred_masks[i],
                qa_gt_masks=gt_masks[i]
            ))
        return results 
    ...
    def predict(self, batch_inputs_dict, batch_data_samples, **kwargs):
        ...
        for i in range(len(batch_data_samples)):
            ...
            sp_gt_masks.append(batch_data_samples[i].gt_pts_seg.sp_gt_seg)
        ...
        x = self.decoder(x, scene_names=scene_names, qa_data=selected_qa_data, point_positions=point_positions)
        results_list = self.qa_predict_by_feat(x, sp_pts_masks, sp_gt_masks)
        for i, data_sample in enumerate(batch_data_samples):
            data_sample.pred_pts_seg = results_list[i]

        # [mask dump] optionally persist predicted/GT per-point masks for visualization.
        # Toggle via env MASK_DUMP_DIR or `--cfg-options model.decoder.save_viz_dir=<dir>`.
        import os as _os
        _dump_dir = _os.environ.get('MASK_DUMP_DIR') or getattr(self.decoder, 'save_viz_dir', None)
        if _dump_dir is not None:
            import numpy as _np
            _os.makedirs(_dump_dir, exist_ok=True)
            for i in range(len(batch_data_samples)):
                _sn = scene_names[i]
                _pred = results_list[i].qa_pred_masks
                _gt = results_list[i].qa_gt_masks
                _np.save(_os.path.join(_dump_dir, f'{_sn}_qa_pred_masks.npy'),
                         _np.array(_pred, dtype=object), allow_pickle=True)
                _np.save(_os.path.join(_dump_dir, f'{_sn}_qa_gt_masks.npy'),
                         _np.array([m.cpu().numpy() if hasattr(m,'cpu') else _np.asarray(m) for m in _gt], dtype=object),
                         allow_pickle=True)
        return batch_data_samples
...
@MODELS.register_module()
class UniSeg3D_Reason(UniSeg3DMixin, Base3DDetector):
    r"""UniSeg3D for ScanNet dataset.
    ...
    def _freeze_non_reason_parameters(self):
        for p in self.parameters():
            p.requires_grad = False

        # 2) Set frozen parts to eval to avoid BN statistics updates
        # with torch.no_grad(): # TODO ONLY TEST 
        #     self.unet.eval()
        #     self.lang.eval()
        #     self.input_conv.eval()
        #     self.output_layer.eval()
        #     if hasattr(self, 'type_embedding'):
        #         self.type_embedding.eval()
        #     if hasattr(self, 'pred_iou_proj'):
        #         self.pred_iou_proj.eval()

        # 3) Enable training only for LoRA and mapping modules related to reasoning
        if hasattr(self, 'decoder'):
            for name, p in self.decoder.named_parameters():
                if 'reason_model' in name and 'lora_' in name:
                    p.requires_grad = True
                elif ('point2token' in name) or ('token2point' in name):
                    p.requires_grad = True
    ...
    def loss(self, batch_inputs_dict, batch_data_samples, **kwargs):
        ...
        # self._freeze_non_reason_parameters()
        batch_offsets = [0]
        superpoint_bias = 0
        sp_gt_instances = []
        sp_pts_masks = []
        ...
        x = self.decoder(x, queries, scene_names=scene_names, interaction_masks=interaction_masks, point_positions=point_positions)
        language_loss = x['language_losses']
        if 'text_token' in batch_inputs_dict.keys():
            x, text_prompt_outputs = self.split_original_and_text_prompt_queries(x, batch_inputs_dict)
        if 'point_prompts' in batch_inputs_dict.keys():
            x, point_prompt_outputs = self.split_original_and_point_prompt_queries(x, batch_inputs_dict)
        ...
        # loss = self.criterion(language_loss=language_loss)
        loss = self.criterion(x, sp_gt_instances, point_prompt_outputs, point_prompt_instances, text_prompt_outputs, text_prompt_instances, language_loss=language_loss)
        return loss
    ...
    def predict(self, batch_inputs_dict, batch_data_samples, **kwargs):
        ...
        scene_names = batch_inputs_dict['scene_name']
        x = self.decoder(x, queries, scene_names, interaction_masks=interaction_masks, point_positions=point_positions)
...
#         loss = self.criterion(language_loss)
#         return loss
...
#         x = self.decoder(x, queries, scene_names, interaction_masks=interaction_masks)
...
@MODELS.register_module()
class UniSeg3D_Eval_PartGlamm(UniSeg3DMixin, Base3DDetector):
    r"""UniSeg3D for ScanNet dataset.
    ...
    def extract_words_from_specific_text(self, text):
        import re
    ...
    def add_qa_prompt_queries(self, batch_inputs_dict):

        selected_qa = batch_inputs_dict['selected_qa_data']
        qa_queries = []

        for i_qa in range(len(selected_qa)):

            if self.prompt_type == 'answer_words':
                answer = selected_qa[i_qa]['answer'][0]
                words = self.extract_words_from_specific_text(answer)
            elif self.prompt_type == 'answer_text':
                words = selected_qa[i_qa]['answer'][0]
            elif self.prompt_type == 'question_text':
                words = selected_qa[i_qa]['question'][0]
            else:
                raise ValueError(f"Invalid prompt type: {self.prompt_type}")
            if len(words) == 0:
                continue
            batch_text_prompts = self.lang(words)
            if 'text' in self.prompt_type:
                batch_text_prompts = torch.cat(batch_text_prompts, dim=0).mean(0, keepdim=True)
            else:
                batch_text_prompts = torch.cat(batch_text_prompts, dim=0)
            qa_queries.append(batch_text_prompts)
    ...
    def qa_predict_by_feat(self, out, superpoints, gt_masks):
        ...
        if len(out['masks']) == 0 or len(gt_masks) == 0:
            return [PointData(qa_pred_masks=[], qa_gt_masks=[]) for _ in range(max(len(gt_masks), 1))]
    ...
    def predict(self, batch_inputs_dict, batch_data_samples, **kwargs):
        ...
        if 'selected_qa_data' in batch_inputs_dict.keys():
            queries = self.add_qa_prompt_queries(batch_inputs_dict)
        ...
        for i, data_sample in enumerate(batch_data_samples):
            if i < len(results_list):
                ...
                data_sample.pred_pts_seg = PointData(qa_pred_masks=[], qa_gt_masks=[])
        return batch_data_samples